...
const SecurityMetrics = () => {
  ...
  // Get security score color using security color scheme
  const getSecurityScoreColor = (score: number) => {
    if (score >= 90) return 'text-security-secure';
    if (score >= 70) return 'text-security-low';
    if (score >= 50) return 'text-security-medium';
    return 'text-security-critical';
  };

  // Get vulnerability count color using security color scheme
  const getVulnCountColor = (count: number) => {
    if (count === 0) return 'text-security-secure';
    if (count <= 5) return 'text-security-low';
    if (count <= 20) return 'text-security-medium';
    return 'text-security-critical';
  };

  // Get trend direction indicator using security colors
  const getTrendIcon = (direction: string) => {
    switch (direction) {
      case 'improving': return <TrendingUp className="h-4 w-4 text-security-secure" />;
      case 'declining': return <TrendingUp className="h-4 w-4 text-security-critical rotate-180" />;
      default: return <Activity className="h-4 w-4 text-security-scanning" />;
    }
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <Shield className="h-4 w-4 text-security-secure" />
          </CardHeader>
          <CardContent>
          ...
          <CardContent>
            ...
            <div className="flex space-x-2 mt-1">
              {metrics.criticalCount > 0 && (
                <Badge variant="destructive" className="text-xs bg-security-critical text-white">
                  {metrics.criticalCount} Critical
                </Badge>
              )}
              {metrics.highCount > 0 && (
                <Badge variant="secondary" className="text-xs bg-security-high text-white">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Scan Status</CardTitle>
            <Clock className="h-4 w-4 text-security-scanning" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold capitalize text-security-scanning">
              {metrics.scanStatus}
            </div>
            <p className="text-xs text-muted-foreground">
